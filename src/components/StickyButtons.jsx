...
export default function StickyButtons() {
    ...
    const { chatHistory, loading, handleSendMessage } = useGeminiChat();
    ...
    return (
        <>
            {/* Botones flotantes (WhatsApp, Mercado Libre, Gemini) */}
            <a
                            ...
                            chatHistory.map((msg, index) => (
                                <div key={index} className={`mb-2 p-2 rounded-lg max-w-[85%] ${msg.role === 'user' ? 'bg-blue-500 text-white self-end ml-auto' : 'bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 self-start mr-auto'}`}>
                                    <p className="font-semibold mb-1 flex items-center gap-2">
                                        {msg.role === 'user' ? <FaUser className="text-white" /> : <FaRobot className="text-gray-800 dark:text-gray-200" />}
